...
- **Test the grounding boundary.** `/memory/ask` used to refuse outright when retrieval came back empty. It no longer does — that made every generally-answerable question ("what should I do about burnout?") a dead end. The guarantee is now narrower and must be tested as such: Yamin may answer from its own understanding, but must never assert a fact about the *user's life* that no retrieved note contains. Concretely: with nothing retrieved, no user note text reaches the prompt and `sources` is empty; when retrieval falls back to recent notes, the prompt says they are background rather than matches.
